...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($user) {
        ...
        $stmt = $pdo->prepare("UPDATE users SET login_code = ?, code_expires_at = ? WHERE id = ?");
        $stmt->execute([$raw_code, $expires, $user['id']]);
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = "Dein Login-Code";
            
            // DAS NEUE E-MAIL DESIGN FÜR LOGIN
            $mail_html = '
            <!DOCTYPE html>
            <html>
                ...
                <div style="background-color: #0f0f13; padding: 40px 20px; color: #ffffff; text-align: center;">
                    <div style="max-width: 500px; margin: 0 auto; background-color: #1a1a24; border: 1px solid #333344; border-radius: 16px; padding: 40px 30px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
                        ...
                        <p style="font-size: 16px; color: #cccccc; line-height: 1.6; margin-bottom: 30px;">
                            Hallo ' . htmlspecialchars($user['username']) . ', hier ist dein angeforderter Login-Code. Bitte gib ihn auf der Website ein, um fortzufahren:
                        </p>
                        ...
                        <p style="font-size: 13px; color: #777777; margin-top: 30px; margin-bottom: 0;">
                            Dieser Code ist für 15 Minuten gültig.<br>Falls du ihn nicht angefordert hast, kannst du diese E-Mail einfach ignorieren.
                        </p>
                    </div>
                </div>
            </body>
        ...
        $error = "E-Mail unbekannt. Bitte registriere dich zuerst."; 
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="container">
        ...
        <section class="glass-card auth-card">
            ...
            <form action="login.php" method="POST">
                <div class="form-group"><label>Deine E-Mail Adresse</label><input type="email" name="email" class="form-control" required placeholder="deine@email.de"></div>
                <button type="submit" class="buy-btn" style="width:100%;">Code anfordern</button>
            </form>
            <p style="margin-top:20px;">Neu hier? <a href="register.php" style="color:var(--glow-color-1);">Registrieren</a></p>
        </section>
    </div>
